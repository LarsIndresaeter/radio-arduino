...
it to the `mqtt broker`. The client will subscribe to a topic on the broker and
execute commands.

The best way to experiment with this is to set up a mqtt broker on localhost and
use the command `raduino mqtt` which will print a help menu
