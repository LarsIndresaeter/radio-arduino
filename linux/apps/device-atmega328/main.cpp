...
void parseOpt(int argc, char* argv[], monitor& mon, LinuxCryptoHandler& cryptoHandler)
{
    ...
    while ((option = getopt(argc, argv, "K:N:AbBCgGjJy:p:P:qQY:zZs:S:tTh")) != -1) {
        ...
        case 'C':
            std::cout << mon.get<>(
                RaduinoCommandWakeup(true, radioAddress), static_cast<std::chrono::milliseconds>(12000))
                      << std::endl;
            break;
        case 'A': {
            while (1) {
                mon.get<>(RaduinoCommandWakeup(true, radioAddress), static_cast<std::chrono::milliseconds>(12000));
