...
void print_usage()
{
    ...
    std::cout << "           -F <key> : set data encryption key on node" << std::endl;
    std::cout << "           -g <key> : set transport encryption key on gateway" << std::endl;
    std::cout << "           -G <key> : set transport encryption key on node" << std::endl;
...
void parseOpt(int argc, char* argv[], monitor& mon, LinuxCryptoHandler& cryptoHandler)
{
    ...
    while ((option = getopt(argc, argv, "aAbBK:N:c:C:f:F:g:G:lLiImMo:xXrRdeE:DpPt:T:usSUvVzZk:w:h")) != -1) {
        ...
        case 'B': {
            ...
            std::cout << " : " << std::to_string(1000 * active_time / (sleep_time + active_time)) << "/1000"
                      << std::endl;
        } break;
        case 'K': {
            std::string s(optarg);
