...
int timeMs()
{
    ...
    int ms = (duration_cast<milliseconds>(system_clock::now().time_since_epoch()).count()) % 1000;
    return ms;
}
...
void readCurrentAndVoltage(monitor& mon, mqtt::async_client& mqtt_client, int samples)
{
    ...
    for (int i = 0; i <= samples; i++) {
        ...
        if (currentData.size() > 2) {
            float I_avg = std::reduce(currentData.begin(), currentData.end()) / currentData.size();
            float V_avg = std::reduce(voltageData.begin(), voltageData.end()) / voltageData.size();
