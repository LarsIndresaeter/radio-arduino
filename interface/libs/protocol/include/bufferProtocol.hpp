...
public:
    ...
    std::vector<uint8_t> parseBinary(std::vector<uint8_t>& inputBuffer)
    {
        ...
        if (inputBuffer.at(PROTOCOL::HEADER::SYNC1_OFFSET)
        ...
        uint8_t protocolVersion
            = inputBuffer.at(PROTOCOL::HEADER::VERSION_OFFSET);
        uint8_t length = inputBuffer.at(PROTOCOL::HEADER::LENGTH_OFFSET);

        if (inputBuffer.size()
            < (PROTOCOL::HEADER::LENGTH + length
        ...
        uint32_t checksumReceived
            = inputBuffer.at(PROTOCOL::HEADER::LENGTH + length) << 24;
        checksumReceived
            |= inputBuffer.at(PROTOCOL::HEADER::LENGTH + length + 1) << 16;
        checksumReceived
            |= inputBuffer.at(PROTOCOL::HEADER::LENGTH + length + 2) << 8;
        checksumReceived
            |= inputBuffer.at(PROTOCOL::HEADER::LENGTH + length + 3);

        uint32_t checksumCalculated = 0;
        uint8_t* p = inputBuffer.data();
        CRC32_calculate(
            &p[PROTOCOL::PAYLOAD::PAYLOAD_OFFSET], length, &checksumCalculated);
        ...
        for (int i = 0; i
             < (PROTOCOL::HEADER::LENGTH + length + PROTOCOL::CHECKSUM::LENGTH);
             i++) {
            inputBuffer.erase(inputBuffer.begin());
        }
    ...
    std::vector<uint8_t> createBinaryCommand(
...
|| protocol_version == static_cast<uint8_t>(PROTOCOL::HEADER::VERSION::RADIO_ENCRYPTED_BINARY_AND_TEXT)
            ...
            command.resize(
                PROTOCOL::HEADER::LENGTH + PROTOCOL::CHECKSUM::LENGTH
                + payload.size() + PROTOCOL::ENCRYPTED::CRYPTO_OVERHEAD);
