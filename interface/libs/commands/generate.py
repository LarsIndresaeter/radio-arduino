...
def generateDocumentationReadme():
    ...
    listOfCommandsFile = commandDocumentationDirectory + "README.md"

    with open(listOfCommandsFile, 'w') as outfile:
        outfile.write("# commands\n")
        outfile.write("\n")

def generateDocumentation(commandId, commandName, 
                 commandPayloadByteNames, 
                 responsePayloadByteNames):
    ...
    with open(listOfCommandsFile, 'a') as outfile:
    ...
    with open(commandDocumentationFile, 'w') as outfile:
        ...
        items = len(responsePayloadByteNames)
        if items > 0:
...
def generateCommandAndPayloadFile(commandId, commandName, 
    ...
    generateDocumentation(commandId, commandName, commandPayloadByteNames, responsePayloadByteNames)
...
def commandDefinitions():
    ...
    generateCommandAndPayloadFile(19, "ws2812b", ["red[8]", "green[8]", "blue[8]"], [])
    ...
    generateCommandAndPayloadFile(25, "nrf24l01_read", ["length", "data[32]"], ["length", "data[32]"])
    generateCommandAndPayloadFile(26, "nrf24l01_write", ["length", "data[32]"], ["length", "data[32]"])
