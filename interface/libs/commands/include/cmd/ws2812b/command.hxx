...
public:
              ...
              COMMANDS::WS2812B::COMMAND_LENGTH)
    {
        ...
        for (uint8_t i = 0; i < sizeof(command.red); i++) {
            if (i >= red.size()) {
                break;
            }
            m_payload.at(
        ...
        for (uint8_t i = 0; i < sizeof(command.green); i++) {
            if (i >= green.size()) {
                break;
            }
            m_payload.at(
        ...
        for (uint8_t i = 0; i < sizeof(command.blue); i++) {
            if (i >= blue.size()) {
                break;
            }
            m_payload.at(
              ...
              COMMANDS::WS2812B::COMMAND_LENGTH)
    {
        ...
        for (uint8_t i = 0; i < red.size() && i < 8; i++) {
            m_payload.at(
                offsetof(COMMANDS::WS2812B::command_t, red[0]) + i)
                = red.at(i);
        }

        for (uint8_t i = 0; i < green.size() && i < 8; i++) {
            m_payload.at(
                offsetof(COMMANDS::WS2812B::command_t, green[0]) + i)
                = green.at(i);
        }

        for (uint8_t i = 0; i < blue.size() && i < 8; i++) {
            m_payload.at(
                offsetof(COMMANDS::WS2812B::command_t, blue[0]) + i)
                = blue.at(i);
        }
