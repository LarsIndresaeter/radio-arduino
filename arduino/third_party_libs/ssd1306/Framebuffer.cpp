...
Framebuffer::Framebuffer() { this->clear(); }
    ...
    for (uint8_t current_y = 0; current_y < height; current_y++) {
        for (uint8_t current_x = 0; current_x < width; current_x++) {
            ...
            if (current_byte & (128 >> (current_x & 7))) {
                this->drawPixel(current_y + pos_x, -current_x + pos_y, 1);
            }
            else {
                this->drawPixel(current_y + pos_x, -current_x + pos_y, 0);
            }
        }
    }
}
...
void Framebuffer::drawChar(uint8_t x, uint8_t y, uint8_t c)
{
    drawBitmap(&Font5x8[(c - 0x20) * 6 + 1], 5, 8, x * 8, (y + 1) * 8);
}
...
void Framebuffer::invert(uint8_t status) { this->oled.invert(status); }

void Framebuffer::show() { this->oled.sendFramebuffer(this->buffer); }
