...
const uint8_t rCON[15] = { 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0xD4, 0x9A };
        ...
        for (int j = 0; j < 4; j++) {
            for (int i = 0; i < 4; i++) {
                if (subMat[z][i] == 0x01) { // x1
                    ...
                    tempBox[z][j]
                        ^= MultiplyBy2(MultiplyBy2(MultiplyBy2((*text)[i][j])) ^ (*text)[i][j]) ^ (*text)[i][j]; // x11
                }
                else if (subMat[z][i] == 0x0D) {
                    tempBox[z][j]
                        ^= MultiplyBy2(MultiplyBy2(MultiplyBy2((*text)[i][j]) ^ (*text)[i][j])) ^ (*text)[i][j]; // x13
                }
                else if (subMat[z][i] == 0x0E) {
                    tempBox[z][j]
                        ^= MultiplyBy2(MultiplyBy2(MultiplyBy2((*text)[i][j]) ^ (*text)[i][j]) ^ (*text)[i][j]); // x14
                }
            }
        }
    }
