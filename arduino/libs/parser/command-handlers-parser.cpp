...
void commandSetKey(uint8_t* commandPayload, uint8_t* responsePayload)
{
    ...
    if (PARSER::lastReceivedCommandWasEncrypted()) {
        if (command.keyId == 'D') {
            EEPROM_DATA_STORE::writeToSpareAndSetAsActive(
                offsetof(eeprom_data_t, dataEncryptionKey), &command.keyValue[0], 16);
        }
        else if (command.keyId == 'T') {
            EEPROM_DATA_STORE::writeToSpareAndSetAsActive(
                offsetof(eeprom_data_t, transportEncryptionKey), &command.keyValue[0], 16);
        }
    }
